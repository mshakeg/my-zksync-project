...
contract Factory2WithConstructorAndLibrary {
    ChildWithConstructorAndLibrary[] public children;

    function createChild(uint _data) public {
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, _data));
        ChildWithConstructorAndLibrary child = new ChildWithConstructorAndLibrary{salt: salt}(msg.sender, _data);
        children.push(child);
    }
